        ...
        with col1:
            ...
            if batter_names:
                ...
                if batter_search:
                    ...
                    filtered = batter_names
                # show top 200 to avoid huge lists
                filtered = filtered[:200]
                if filtered:
                    batter_name = st.selectbox("Player", options=filtered, index=0)
                else:
                    st.write("No batters match your search")
                    batter_name = ""
            else:
                batter_name = st.text_input("Batter name (no id)", value="")

            # Pitcher search + filtered selectbox
            if pitcher_names:
                pitcher_search = st.text_input("Search pitcher", value="")
                if pitcher_search:
                    p_filtered = [n for n in pitcher_names if pitcher_search.lower() in n.lower()]
                else:
                    p_filtered = pitcher_names
                p_filtered = p_filtered[:200]
                if p_filtered:
                    pitcher_name = st.selectbox("Pitcher", options=p_filtered, index=0)
                else:
                    st.write("No pitchers match your search")
                    pitcher_name = ""
            else:
